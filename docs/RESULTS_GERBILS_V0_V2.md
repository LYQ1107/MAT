...
| SLEAP_OFFICIAL_RANDOM_SPLIT_BASELINE (formal) | SLEAP-NN 0.3.3 bottomup，GPU 1，计划 50 epochs（运行中；已到 epoch 49 / `global_step=10000`，非最终结果） | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN |
...
- formal checkpoint 目录：`MAT_workspace/runs/sleap_gerbils_pose_full/`；GPU 1 的正式训练命令、PID 和实时 log 在 `formal_training_launch.json` 及其模型目录，当前仅确认到 epoch 49 / `global_step=10000`，不作为最终结果。
...
当前可称为：真实 SLEAP 数据适配、公开 CLI schema 审计、2-epoch CPU pose smoke、formal GPU 训练（进行中）、test/clip 格式链和 MAT 核心实现已落地。当前不能称为：完整 pose baseline 指标、跨日 persistent-ID accuracy、H/A 人工核验时长、B1/B2 改善、strict longitudinal generalization 或论文级结果。formal test 成功且非零预测后，才按 S0-only mapping 运行 B0；没有跨日人工映射或姿态 GT 时继续保持对应阻塞。
