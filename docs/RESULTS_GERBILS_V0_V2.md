...
| SLEAP_OFFICIAL_RANDOM_SPLIT_BASELINE (formal) | SLEAP-NN 0.3.3 bottomup，GPU 1，计划 50 epochs（运行中；已到 epoch 43 / `global_step=8800`，非最终结果） | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN |
...
- formal checkpoint 目录：`MAT_workspace/runs/sleap_gerbils_pose_full/`；GPU 1 的正式训练命令、PID 和实时 log 在 `formal_training_launch.json` 及其模型目录，当前仅确认到 epoch 43 / `global_step=8800`，不作为最终结果。
