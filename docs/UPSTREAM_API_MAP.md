...
| WildlifeTools | `2f22214c8c331ca4174ce231c23de0add7718ee2` | `DeepFeatures(model, batch_size=128, num_workers=1, device='cpu', cache_path=None)`；`forward_batch` 内 `torch.no_grad()`，仅冻结推断。MAT 另核验了官方 `BVRA/MegaDescriptor-T-224` revision `3ea58ff6c6195bc748bb86c111ff40c32bdddcba` 的 `swin_tiny_patch4_window7_224`/768 维 config 与本地 timm state-key remap；未把 DeepFeatures 当训练 API。 |
...
未核验内容：DLC 本机安装版本、模型 zoo 权重、SuperAnimal 输出实际 h5/JSON 键、ByteTrack 补丁后未修改版等价性，以及所有数据的跨日映射。MAT 后端在缺少本地校验资产时明确失败，不猜函数或静默联网。
