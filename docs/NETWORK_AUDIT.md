...
结论：`application_proxy_bypass=true` 只适用于 `DirectOnlyPolicy.child_env` 子进程；`route_status=DIRECT_ROUTE_UNVERIFIED`，不是“绝对直连”。因此 Rat ID、PigReID、PigTracking、MultiCamCows 和所有未知模型权重均为 `BLOCKED_DIRECT_ROUTE`，直到管理员/用户提供经核验的直连出口、可信镜像或授权本地文件。不会偷偷回退现有代理。
